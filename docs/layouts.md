...
| `PageHeaderLayout`        | `PageHeader`                   | `pageHeader: PageHeaderProps`; optional `banner?: BannerProps` (stay-pinned under the header; does not auto-hide); always includes `AutoSaveIndicator` in the header actions area (hidden while idle); body → **`children`**                                                                                                                                                                                                                                                                                                                |
| `StepperLayout`           | `WorkflowHeader` + steps       | `title`, `onCancel`, `completeLabel`, `onComplete`, `onSaveAndExit?`, `isDirty?`, `allowNavigation?` flattened onto `StepperLayoutProps`, optional `banner?: BannerProps` (stay-pinned under the header), `steps: StepperLayoutStep[]` (label/`primaryDisabled?`/`disabled?`/content — no `value`, it's derived from `label`) — `primaryDisabled` gates Continue/Complete (`boolean \| ReactNode`, same as `Button.disabled`); `disabled` only locks the tab. Active step's `content` is the body and need not be `FormSectionLayout`; `defaultStep?` picks the initial step; standalone, only ever under `EnvironmentBannerLayout` (see rule 3). `aiMode?` paints the body AI background and uses the `ai` Continue/Complete variant — pair with `FormSectionLayout` `aiMode` for the card + gradient title. Step `rightPaneTriggers?` (`{ icon, label, content }[]`) hosts the document-scroll pane at the workflow from the active step and renders icon triggers (desktop: floating stack; `sm`: header right slot). Triggers slide off the right edge while the pane is open and return when it closes; the pane closes on step change; close via `RightPanePanel`'s header X — do not also set `withRightPane` on a step body. |
| `FocusedFormLayout`       | `WorkflowHeader` (no steps)    | Same flattened chrome as `StepperLayout` (`isDirty?` / `allowNavigation?` / `banner?` included), plus `primaryDisabled?` (gates Create/Save; a ReactNode is the tooltip), `aiMode?` (page wash + `ai` CTA), `rightPaneTriggers?` (`{ icon, label, content }[]`, layout-level), and body → **`children`** (typically `FormSectionLayout`). Does **not** own JumpLinks — pass `withJumpLinks` on the body. Standalone, only ever under `EnvironmentBannerLayout` (see rule 3). |
...
scroll-up. (Workflow layout headers are the exception — see below.) Optional `banner` on
`PageHeaderLayout` / `StepperLayout` / `FocusedFormLayout` is **stay-pinned** — it does not auto-hide
with the page header. When the header or navbar hides, their height vars collapse to `0px` and the
banner slides up to sit under the remaining chrome.
...
workflow chrome: pass the body as **`children`** (typically `FormSectionLayout`). JumpLinks are owned by
**`FormSectionLayout`** (`withJumpLinks`, `includeTitleJumpLink`, section `excludeJumpLink`) — use the same body in a Stepper step or under FocusedForm.
`aiMode` on the workflow paints a full-bleed wash; pair with `FormSectionLayout` `aiMode` for the card
+ gradient title. **`rightPaneTriggers`** on `FocusedFormLayout` or on each `StepperLayout` step hosts the document-scroll pane at the workflow and renders the trigger chrome (slides off-screen to the right while the pane is open) — do **not** also set `withRightPane` on the body. On a stepper, chrome comes from the active step and the pane closes when the step changes. Without triggers, form bodies still use `FormSectionLayout withRightPane` and table bodies `GridTableLayout withRightPane`. Do **not** nest another right-pane host around those bodies.
...
The layouts share a **document-scroll** model: the environment banner, navbar, and page header pin via
`position: fixed` (so horizontal document scroll cannot move them). The side nav rail is `sticky`. Chrome
communicates through CSS custom properties so wide content (e.g. large tables) uses the document
scrollbars while that chrome stays in place:
...
- `PageHeaderLayout` reads those and publishes its own height (`--beam-page-header-layout-height`). Optional `banner` is in-flow `position: sticky` (same pattern as `GridTableLayout` table actions — not `fixed`), publishes `--beam-page-banner-height` while mounted, and sticks at `pageBannerChromeTop()` (env + navbar + page header). `stickyNavAndHeaderOffset` / `stickyTableHeaderOffset` include the page banner so table actions, JumpLinks, and table headers sit below it.
- `GridTableLayout` — **inside `DocumentScrollLayoutProvider`** (e.g. under `NavbarLayout` / `PageHeaderLayout`): when filters, search, or edit-columns are shown, measures and publishes the table actions toolbar height (`--beam-table-actions-height`), pins table actions with the other document-scroll chrome, and renders the table inline for document scroll. **`GridTable`** reads these vars so its sticky header sits below the environment banner + navbar + page header + page banner + table actions and its sticky columns sit right of the side nav rail. Pass **`withRightPane`** (`true` / px / `{ width? }`) to opt the table into **`DocumentScrollOverlayRightPaneLayout`** around **only the table body** (table actions stay outside so they remain full-bleed); open the pane via **`useRightPane`** / **`useRightPaneActions`**. For virtual tables, prefer **`useRightPaneActions()`** in row click handlers so open/close does not re-render the table tree. The layout publishes scoped `--beam-right-pane-width` for sticky **right** columns when the pane is open. Omit `withRightPane` when the page does not use a detail pane. Do **not** wrap the whole `GridTableLayout` (or legacy **`RightPaneLayout`**) for this pattern. **Outside document-scroll layouts**, `GridTableLayout` does not pin table actions; it wraps the table in **`ScrollableParent` / `ScrollableContent`** (with `virtualized` when `as="virtual"`) so scrolling stays in the legacy page scroll region — keep using **`RightPaneLayout`** there until the page migrates.
- `DocumentScrollOverlayRightPaneLayout` — the document-scroll right pane (`children` = main content; pane body from `openRightPane`). Compose **at the body that knows its shape**, or on the workflow when `rightPaneTriggers` is set (`FocusedFormLayout`, or the active `StepperLayout` step — the layout hosts the pane and chrome; the pane closes when the step changes). At most **one** host in the tree (dev warning if nested). Open/close state lives in the right-pane external store so row swaps and pane toggles do not re-render the host's `children`. **Desktop / `md+`:** `position: fixed` pane portaled to `document.body`, anchored for top/height (follows scroll until pinned below `stickyTableHeaderOffset()`). Width is `min(paneWidth, documentScrollChromeWidth())` via `documentScrollRightPaneWidthCss`. While open, publishes `--beam-right-pane-width` (sticky-right columns) and `--beam-floating-right-offset` on `document.documentElement` (e.g. `DocumentScrollToTopButton` via `getFloatingRightOffset`). A pane-width spacer (always mounted on `md+`, `0px` while closed) CSS-transitions to the pane width with the slide so centered content eases instead of jumping; it grows the document so content under the pane stays reachable. While open on `md+`, publishes `--beam-right-pane-content-min: 480px` (`0px` when closed) for the form/shell column to read — not overlay `main` — so JumpLinks sit outside the floor (wide leftover = side-by-side; tight leftover = horizontal scroll). Page-header chrome is `position: fixed` so horizontal scroll cannot move it. The overlay host shell does not subscribe to open state — chrome updates run in a sibling subtree. **`sm`:** full-bleed portal overlay below the env banner (same pattern as `NavbarMobileMenu`); scroll locked via `usePreventScroll`; no spacer / min-width / width vars stay `0px`. Optional pane chrome: **`RightPanePanel`** (title + close). Hooks: **`useRightPane()`** (open, close, `isRightPaneOpen`); **`useRightPaneActions()`** (open/close only — use in table row handlers). Used by `GridTableLayout` around the table body and by `CenteredLayout` / `FormSectionLayout` for form panes.
- `StepperLayout` / `FocusedFormLayout` publish `--beam-page-header-layout-height` the same way `PageHeaderLayout` does (so `stickyNavAndHeaderOffset` / `stickyTableHeaderOffset` / `GridTable` keep working unchanged inside a workflow page). Optional `banner` publishes `--beam-page-banner-height` the same way. Their headers do **not** auto-hide — they are `position: fixed` to the viewport (chrome width, same idea as the environment banner and a revealed `NavbarLayout`), with an in-flow spacer for height, so they do not ride horizontal document scroll. They do no scroll-position tracking of their own. `StepperLayout`'s stepper tabs collapse to a condensed indicator bar on the `sm` breakpoint only; there is no scroll-driven collapse at `md`+ (that behavior was removed pending a revisit — `useScrollCollapse` is kept but unused). On the `sm` breakpoint, action buttons move to a sticky mobile footer instead, whose height they publish as `--beam-workflow-layout-footer-height`. `getFloatingBottomOffset(basePx?)` reads that var so bottom-anchored floating chrome (e.g. `DocumentScrollToTopButton`) stacks above the footer instead of being covered by it — the same pattern future bottom-anchored elements should use. `aiMode` paints a full-bleed `aiBackground` wash on the body and uses the `ai` Continue/Complete variant — pair with `FormSectionLayout` `aiMode` for the `AiCard` + gradient title. `FormSectionLayout` JumpLinks (`withJumpLinks`) are hidden on `sm`. `rightPaneTriggers` on `FocusedFormLayout` or on each `StepperLayout` step hosts the pane at the workflow (active step only; pane closes on step change); without it, form bodies use `FormSectionLayout withRightPane` and table bodies `GridTableLayout withRightPane`.
