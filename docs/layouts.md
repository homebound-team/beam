...
**`FormSectionLayout`** (`withJumpLinks`, section `excludeJumpLink`, `withRightPane` — default mode
**`auto`**, opened via **`useRightPane`**) — use the same body in a Stepper step or under FocusedForm.
...
two document-scroll right-pane hosts, and do **not** wrap in legacy **`RightPaneLayout`**.
...
- `GridTableLayout` — **inside `DocumentScrollLayoutProvider`** (e.g. under `NavbarLayout` / `PageHeaderLayout`): when filters, search, or edit-columns are shown, measures and publishes the table actions toolbar height (`--beam-table-actions-height`), pins table actions with the other document-scroll chrome, and renders the table inline for document scroll. **`GridTable`** reads these vars so its sticky header sits below the environment banner + navbar + page header + table actions and its sticky columns sit right of the side nav rail. Pass **`withRightPane`** (`true` / px / `{ width?, mode? }`; default mode **`overlay`**) to opt the table into **`DocumentScrollRightPaneLayout`** around **only the table body** (table actions stay outside so they remain full-bleed); open the pane via **`useRightPane`** / **`useRightPaneActions`**. For virtual tables, prefer **`useRightPaneActions()`** in row click handlers so open/close does not re-render the table tree. The layout publishes scoped `--beam-right-pane-width` for sticky **right** columns when the pane is open. Omit `withRightPane` when the page does not use a detail pane. Do **not** wrap the whole `GridTableLayout` (or legacy **`RightPaneLayout`**) for this pattern. **Outside document-scroll layouts**, `GridTableLayout` does not pin table actions; it wraps the table in **`ScrollableParent` / `ScrollableContent`** (with `virtualized` when `as="virtual"`) so scrolling stays in the legacy page scroll region — keep using **`RightPaneLayout`** there until the page migrates.
- `DocumentScrollRightPaneLayout` — document-scroll right pane host (`children` = main content; pane body from `openRightPane`). Compose **at the body that knows its shape** — never on `StepperLayout` / `WorkflowPageLayout` / `FocusedFormLayout`. At most **one** host in the tree (dev warning if nested). Open/close state lives in external stores on **`RightPaneProvider`** so row swaps and pane toggles do not re-render the host's `children`. **Modes (desktop / `md+`):**
  - **`overlay`** (GridTable default): **`position: fixed`** pane portaled to `document.body`, anchored to the host for top/height (follows scroll until pinned below `stickyTableHeaderOffset()`). A zero-width **spacer** slot in the host flex row widens only while open so horizontal document scroll can reach columns under the pane; publishes `--beam-right-pane-width` for sticky-right columns while open. The table host shell does not subscribe to open state — chrome updates run in a sibling subtree.
  - **`push`**: the main column shrinks clear of the pane (host flex container stays chrome-wide); pane is an in-flow **`position: sticky`** sibling.
  - **`auto`** (CenteredLayout / FormSectionLayout default): if the centered shell already clears the pane → `clear` (a negative margin pulls the pane into the gutter so the shell does not shift); else → **`push`** (main column shrinks beside the pane, even when tight — no desktop full-screen fallback). Uses deterministic shell / JumpLinks math resolved once on open (no DOM measuring or resize re-evaluation). `FormSectionLayout` passes JumpLinks width into `auto` when the rail is shown; the math reproduces the rail's mirrored `clamp` reservation, so while the form is still page-centered the rail does not change the outcome, and it only counts against the form once the mirror collapses.
  Push/clear panes are in-flow **`position: sticky`** siblings of the main column: they start at the top of the host's content — so in-flow chrome above the host (e.g. a `ContentHeader`) is respected — and pin at `stickyTableHeaderOffset()` once scrolled vertically. **`max-height`** is viewport-relative with internal scroll (`overflow-y: auto`); width `min(paneWidth, documentScrollChromeWidth())`. Publishes `--beam-floating-right-offset` on `document.documentElement` when open on desktop so floating chrome (e.g. `DocumentScrollToTopButton`) can clear the pane. **`sm` only:** full-bleed portal overlay below the env banner (same pattern as `NavbarMobileMenu`); scroll locked via `usePreventScroll`; no split layout / width vars stay `0px`. Optional pane chrome: **`RightPanePanel`** (title + close). Hooks: **`useRightPane()`** (open, close, `isRightPaneOpen`); **`useRightPaneActions()`** (open/close only — use in table row handlers).
