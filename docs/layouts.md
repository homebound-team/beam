...
+ gradient title. **`rightPaneTriggers`** on the workflow hosts the document-scroll pane (so the pane survives step changes) and renders the trigger chrome (slides off-screen to the right while the pane is open) — do **not** also set `withRightPane` on the body. Without triggers, form bodies still use `FormSectionLayout withRightPane` and table bodies `GridTableLayout withRightPane`. Do **not** nest another right-pane host around those bodies.
