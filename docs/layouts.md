...
workflow chrome: pass the body as **`children`** (typically `FormSectionLayout`). JumpLinks and the right pane are both owned by
**`FormSectionLayout`** (`withJumpLinks`, section `excludeJumpLink`, `withRightPane` — default
`reserveScroll: "auto"`) — use the same body in a Stepper step or under FocusedForm.
`aiMode` on the workflow paints a full-bleed wash; pair with `FormSectionLayout` `aiMode` for the card
+ gradient title. **Neither workflow layout takes `withRightPane`** — form bodies use
`FormSectionLayout withRightPane`, table bodies use `GridTableLayout withRightPane`. Do **not** nest
another right-pane host around those bodies.
