...
2. **Preserve nesting order** when all apply: **`EnvironmentBannerLayout` → `NavbarLayout` → `SideNavLayout` → `PageHeaderLayout`**. Put **`CenteredLayout`** / **`FormSectionLayout`** in `PageHeaderLayout` / `StepperLayout` / `FocusedFormLayout` **children** — they are body-width shells, not chrome peers. Page headers stay full-bleed. `CenteredLayout` is a new-layout shell meant for those document-scroll bodies (`ContentInsetProvider` / `insetHandled` is that signal; it can wrap other trees, but product usage is document-scroll). Body chrome in a document-scroll page (`ContentHeader`, `GridTableLayout`'s actions / filters, table gutter columns) applies `pageContentPaddingX` so it clears the viewport edge, and drops it inside `CenteredLayout`, which already insets children. `GridTable` also defaults to `allWhite` on a page: bordered with a rounded header inside that inset shell, square-headered when full-bleed. Pass a `GridStyleDef` to opt out. (Unrelated to `as="card"` / card view.) `PageHeaderLayout` spaces its body's **first child** 24px below the header, unless that child spreads **`selfTopSpaced`** (`data-self-top-spaced`) to say it pads its own top edge. Sticky chrome must do this: a page-body margin scrolls away when the chrome pins, so the inset has to be padding inside the pinned, painted box — `GridTableLayout` marks its wrapper whenever it renders the actions toolbar. `FormSectionLayout` wraps `CenteredLayout size="sm"` (do not wrap it again).
