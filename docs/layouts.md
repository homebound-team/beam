...
| `FocusedFormLayout`       | `WorkflowHeader` (no steps)    | Same flattened chrome as `StepperLayout`, plus `isValid`, `withJumpLinks?` (default true; rail needs 2+ includable sections), `aiMode?` (page wash + `ai` CTA + forwarded to `form` for `AiCard` + gradient title), `withRightPane?` (`true` / px / `{ width?, mode? }`; default mode `auto`) to wrap **JumpLinks + form** in `DocumentScrollRightPaneLayout` (open via `useRightPane`), and `form` (omit `withRightPane` on the nested form — the layout hosts the pane). JumpLinks come from `form.sections` titles (`excludeJumpLink` to omit). Always uses `FormSectionLayout`. Standalone, only ever under `EnvironmentBannerLayout` (see rule 3). |
| `CenteredLayout`          | centered body-width shell      | `size: "sm" \| "lg"`; body → **`children`**; optional `withRightPane?` (default mode `auto`) wraps the shell in `DocumentScrollRightPaneLayout`. **Not** a chrome peer (see rule 2). Horizontal padding 12px / 24px from `md`, publishes `--beam-layout-content-padding-x` for `layoutContainer` / sticky in-column chrome. `sm` = 720px content (768px shell max); `lg` = 1392px content (1440px shell max). `FormSectionLayout` wraps `CenteredLayout size="sm"` and forwards `withRightPane` — do not wrap `FormSectionLayout` in another `CenteredLayout`. `FormSectionLayout` `aiMode` wraps that content in `AiCard`. |
...
only picks the start). **`FocusedFormLayout`** is the single-form experience: pass `form` (always a
`FormSectionLayout`); JumpLinks are built from section titles when there are two or more includable
sections. `aiMode` paints a full-bleed wash and forwards the form card wrap. Pass **`withRightPane`**
on **`FocusedFormLayout`** (not on the nested form) to host JumpLinks + form in
**`DocumentScrollRightPaneLayout`** (default mode **`auto`**); open via **`useRightPane`**.
**`StepperLayout` has no `withRightPane`** — each step’s body opts in
(`FormSectionLayout` / `CenteredLayout` / `GridTableLayout withRightPane`). Do **not** nest two
document-scroll right-pane hosts. Do **not** wrap in legacy **`RightPaneLayout`**.
...
- `GridTableLayout` — **inside `DocumentScrollLayoutProvider`** (e.g. under `NavbarLayout` / `PageHeaderLayout`): when filters, search, or edit-columns are shown, measures and publishes the table actions toolbar height (`--beam-table-actions-height`), pins table actions with the other document-scroll chrome, and renders the table inline for document scroll. **`GridTable`** reads these vars so its sticky header sits below the environment banner + navbar + page header + table actions and its sticky columns sit right of the side nav rail. Pass **`withRightPane`** (`true` / px / `{ width?, mode? }`; default mode **`overlay`**) to opt the table into **`DocumentScrollRightPaneLayout`** around **only the table body** (table actions stay outside so they remain full-bleed); open the pane via **`useRightPane`**. The layout hosts the fixed pane (pinned below table actions via `stickyTableHeaderOffset`), publishes scoped `--beam-right-pane-width` for sticky **right** columns, and (in `overlay`) adds a matching-width spacer so horizontal document scroll can reach columns under the pane. Omit `withRightPane` when the page does not use a detail pane. Do **not** wrap the whole `GridTableLayout` (or legacy **`RightPaneLayout`**) for this pattern. **Outside document-scroll layouts**, `GridTableLayout` does not pin table actions; it wraps the table in **`ScrollableParent` / `ScrollableContent`** (with `virtualized` when `as="virtual"`) so scrolling stays in the legacy page scroll region — keep using **`RightPaneLayout`** there until the page migrates.
- `DocumentScrollRightPaneLayout` — document-scroll right pane host (`children` = main content; pane body from `openRightPane`). Compose **at the body that knows its shape** — never on `StepperLayout` / `WorkflowPageLayout`. At most **one** host in the tree (dev warning if nested). **Modes (desktop / `md+`):**
  - **`overlay`** (GridTable default): fixed pane + horizontal **spacer** so content under the pane stays reachable; publishes `--beam-right-pane-width` for sticky-right columns.
  - **`push`**: constrain the main column clear of the pane (no spacer).
  - **`auto`** (CenteredLayout / FormSectionLayout / FocusedForm default): if the centered shell already clears the pane → open with **no spacer**; else if remaining width ≥ ~460px → `push`; else → `overlay` (never leave content unreachable). Uses deterministic shell / JumpLinks math (no DOM measuring). FocusedForm passes JumpLinks width into `auto` when the rail is shown.
  Pane `top` / height use `stickyTableHeaderOffset()` / `documentScrollRightPaneHeight()`; width is `min(paneWidth, documentScrollChromeWidth())`. Always publishes `--beam-floating-right-offset` on `document.documentElement` when open on desktop so floating chrome (`DocumentScrollToTopButton`, `RightPaneTrigger`) can clear the pane. **`sm`:** full-bleed portal overlay below the env banner (same pattern as `NavbarMobileMenu`); scroll locked via `usePreventScroll`; no spacer / width vars stay `0px`. Optional chrome: **`RightPanePanel`** (title + responsive close — desktop on the pane’s outer edge, `sm` in the header), **`RightPaneTrigger`** / **`RightPane`** (optional floating trigger when closed).
- `StepperLayout` / `FocusedFormLayout` publish `--beam-page-header-layout-height` the same way `PageHeaderLayout` does (so `stickyNavAndHeaderOffset` / `stickyTableHeaderOffset` / `GridTable` keep working unchanged inside a workflow page), but their headers do **not** auto-hide — they are `position: fixed` to the viewport (chrome width, same idea as the environment banner and a revealed `NavbarLayout`), with an in-flow spacer for height, so they do not ride horizontal document scroll. They do no scroll-position tracking of their own. `StepperLayout`'s stepper tabs collapse to a condensed indicator bar on the `sm` breakpoint only; there is no scroll-driven collapse at `md`+ (that behavior was removed pending a revisit — `useScrollCollapse` is kept but unused). On the `sm` breakpoint, action buttons move to a sticky mobile footer instead, whose height they publish as `--beam-workflow-layout-footer-height`. `getFloatingBottomOffset(basePx?)` reads that var so bottom-anchored floating chrome (e.g. `DocumentScrollToTopButton`) stacks above the footer instead of being covered by it — the same pattern future bottom-anchored elements should use. `aiMode` paints a full-bleed `aiBackground` wash on the body and uses the `ai` Continue/Complete variant; `FocusedFormLayout` also forwards `aiMode` to `FormSectionLayout` for the `AiCard` + gradient title. `FocusedFormLayout`'s JumpLinks rail is hidden on `sm`. **`FocusedFormLayout withRightPane`** hosts JumpLinks + form (default `auto`); do not also set `withRightPane` on the nested form. **`StepperLayout` does not take `withRightPane`** — form steps use `FormSectionLayout withRightPane`, table steps use `GridTableLayout withRightPane`.
