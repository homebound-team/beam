...
export function getSelectCardOptionsCss({
  view,
  hasDescription,
  layout,
  hasImage,
  hasTag,
}: GetSelectCardOptionsCssArgs) {
  if (view === "list") return Css.df.fdc.gap2.w100.$;
  // Horizontal cards fit the icon and text side-by-side, so they need a wider column.
  const cardWidth = layout === "horizontal" ? (hasImage ? 360 : 240) : hasImage || hasDescription || hasTag ? 192 : 142;
