...
import { isPlainDate, jsDateToPlainDate, plainDateToJsDate, todayPlainDate } from "src/utils/plainDate";
import { Temporal } from "temporal-polyfill";

export type DateFieldModeTuple = readonly ["range", DateRange] | readonly ["single", PlainDate];
...
export function formatDate(date: PlainDate | undefined, format: string) {
  if (!date) return "";
  return dateFnsFormat(plainDateToJsDate(date), format);
}

export function formatDateRange(date: DateRange | undefined, format: string) {
  if (!date) return "";
  const { from, to } = date as DateRange;
  const fromFormatted = from ? dateFnsFormat(plainDateToJsDate(from), format) : "";
  const toFormatted = to ? dateFnsFormat(plainDateToJsDate(to), format) : "";
...
export function parseDate(str: string, format: string): PlainDate | undefined {
...
function parseDateString(str: string, format: string): PlainDate | undefined {
  ...
  const parsed = dateFnsParse(str, format, plainDateToJsDate(todayPlainDate()));
...
function isValidJsDate(d: Date | undefined): boolean {
