 ...
 *
 * Reads left-to-right rather than centred like `AiImportBanner` — the copy sits beside its actions so
 * the whole thing stays one row tall above the content being reviewed.
 */
export function AiReviewBanner(props: AiReviewBannerProps) {
  const { title, message, primaryAction, secondaryAction } = props;
  const tid = useTestIds(props, "aiReviewBanner");
  return (
    <AiPanel {...tid}>
