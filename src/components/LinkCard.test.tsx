...
describe("LinkCard", () => {
  it("renders its title and message", async () => {
    const r = await render(
      <LinkCard title="Review 1 changed elevation." message="Describe change" onClick={() => {}} />,
    );
  ...
  it("omits the message when it has none", async () => {
    const r = await render(<LinkCard title="Review 1 changed elevation." onClick={() => {}} />);
    expect(r.query.linkCard_message).not.toBeInTheDocument();
  });

  it("fires its onClick", async () => {
    const onClick = vi.fn();
    const r = await render(<LinkCard title="Review 1 changed elevation." onClick={onClick} />);
  ...
  it("links when given a url", async () => {
    const r = await render(
      <LinkCard title="Review 1 changed elevation." onClick="/plans/1/elevations" />,
      withRouter(),
    );
  ...
  it("names the arrow after the title", async () => {
    const r = await render(<LinkCard title="Review 1 changed elevation." onClick={() => {}} />);
    expect(r.linkCard_action).toHaveAttribute("aria-label", expect.stringContaining("Review 1 changed elevation."));
  });
