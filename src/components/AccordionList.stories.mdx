import { Meta, Canvas, Story, ArgsTable } from "@storybook/addon-docs";
import { AccordionList } from "./AccordionList";

<Meta
  title="Catalog/Accordions/AccordionList"
  component={AccordionList}
  args={{
    accordions: [
      {
        title: "Iron Man",
        children:
          "Genius. Billionaire. Philanthropist. Tony Stark's confidence is only matched by his high-flying abilities as the hero called Iron Man.",
      },
      {
        title: "Captain Marvel",
        children:
          "Carol Danvers becomes one of the universe's most powerful heroes when Earth is caught in the middle of a galactic war between two alien races.",
      },
      {
        title: "Captain America",
        children:
          "Recipient of the Super Soldier serum, World War II hero Steve Rogers fights for American ideals as one of the world’s mightiest heroes and the leader of the Avengers.",
      },
      {
        title: "Thor",
        children:
          "The son of Odin uses his mighty abilities as the God of Thunder to protect his home Asgard and planet Earth alike.",
      },
      {
        title: "Black Widow",
        children:
          "Natasha Romanoff, separated from the now-fractured Avengers, confronts the dark path she took to becoming a spy and assassin, as well as events that followed.",
      },
    ],
  }}
  argTypes={{
    accordions: { table: { control: { disable: true } } },
    allowMultipleExpanded: { table: { defaultValue: { summary: true } } },
    size: {
      table: { defaultValue: { summary: "undefined" } },
      control: { type: "select" },
      options: [undefined, "xs", "sm", "md", "lg"],
    },
  }}
  parameters={{
    design: {
      type: "figma",
      url: "https://www.figma.com/file/aWUE4pPeUTgrYZ4vaTYZQU/%E2%9C%A8Beam-Design-System?node-id=31565%3A99585",
    },
  }}
/>

export const Template = (args) => {
  return <AccordionList {...args} />;
};

# AccordionList

The `AccordionList` component is a group `Accordion` components. This grouping allows the for defining interactions between multiple `Accordion`s. Specifically their size, layout, and whether only one or multiple can be opened at a time.

## Example

<Canvas>
  <Story name="AccordionList">{Template.bind({})}</Story>
</Canvas>

### Properties

Modifying these properties will update the above example.

<ArgsTable story="AccordionList" />
