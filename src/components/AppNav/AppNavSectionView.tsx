...
export function AppNavSectionView(props: AppNavSectionViewProps) {
  const { section, variant, panelCollapsed, showDivider, ...scopeProps } = props;
  const tid = useTestIds(scopeProps);

  // if variant is global, will have right border divider and hide the section label
  const dividerStyles =
    variant === "global" ? Css.pr1.br.bc(Tokens.SurfaceSeparator).$ : Css.pb2.bb.bc(Tokens.SurfaceSeparator).$;
  ...
  return (
    ...
    >
      ...
      <AppNavItems {...scopeProps} items={section.items} variant={variant} panelCollapsed={panelCollapsed} />
    </div>
  );
}
