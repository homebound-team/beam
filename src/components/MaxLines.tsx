...
export function MaxLines({ maxLines, children }: MaxLinesProps) {
  ...
  useLayoutEffect(() => {
    ...
    // If the content overflows, then save the height of the collapsed content and set the `hasMore` state to true.
    // setCollapsedHeight(resizeRef.current.clientHeight);
  ...
  return (
    <div>
      ...
      {hasMore && (
        <button css={Css.db.smMd.$} aria-hidden={true} onClick={() => setExpanded((prev) => !prev)}>
...
const styles: Record<MaxLinesProps["maxLines"], Properties | undefined> = {
  1: Css.lineClamp1.$,
  2: Css.lineClamp2.$,
  3: Css.lineClamp3.$,
  4: Css.lineClamp4.$,
