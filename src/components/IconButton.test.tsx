...
describe("IconButton", () => {
  it("can have a data-testid", async () => {
    ...
    expect(r.baseElement.getElementsByTagName("button")[0].getAttribute("data-testid")).toEqual("remove");
  });

  it("defaults data-testid to the icon", async () => {
    const r = await render(<IconButton icon="trash" />);
    expect(r.baseElement.getElementsByTagName("button")[0].getAttribute("data-testid")).toEqual("trash");
  });
  ...
  it("can accept prefixed test ids", async () => {
    ...
    expect(r.baseElement.getElementsByTagName("button")[0].getAttribute("data-testid")).toEqual("page1_remove");
  });
