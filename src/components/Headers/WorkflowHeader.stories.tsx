    ...
    <WorkflowHeader
      ...
      rightSlot={
        <div css={Css.df.aic.gap1.$}>
          <Button label="Save Draft" variant="secondary" onClick={action("save clicked")} />
          <Button label="Continue" variant="primary" onClick={action("continue clicked")} />
        </div>
      }
...
export function WithBreadcrumbsAndButtons() {
  ...
  return (
    <WorkflowHeader
      ...
      rightSlot={<Button label="Continue" variant="primary" onClick={action("continue clicked")} />}
      stepperTabs={stepperTabs}
    />
  );
}

function createSteps(): StepperTabsStep[] {
  return [
